...
trait ChargeSessionHandler {
  def startSession(
    request: ChargeSessionApi.StartChargeSessionRequest
  ): Task[ChargeSessionApi.StartChargeSessionResponse]
  def stopSession(request: ChargeSessionApi.StopChargeSessionRequest): Task[ChargeSessionApi.StopChargeSessionResponse]
}

class LiveChargeSessionHandler(
  chargingHubClient: ZioChargingHubApi.ChargingHubApiClient,
  chargeSessionRepository: ChargeSessionRepository,
  publisher: SessionPublisher
) extends ChargeSessionHandler {
  override def startSession(
    request: ChargeSessionApi.StartChargeSessionRequest
  ): Task[ChargeSessionApi.StartChargeSessionResponse] =
    zio.Random.nextUUID.flatMap { requestId =>
        ...
        for {
          ...
          res <- if (hubResponse.success) {
                   chargeSessionRepository
                     .upsert(
                       ChargeSessionRepository.ChargeSession(
                     ...
                     .as(
                       ChargeSessionApi
                         .StartChargeSessionResponse(success = hubResponse.success, sessionId = hubResponse.sessionId)
                     )
                 } else
                   ZIO.succeed(
                     ChargeSessionApi.StartChargeSessionResponse(
                       success = hubResponse.success,
                       sessionId = hubResponse.sessionId
                     )
                   )
        } yield res
      )
  ...
  override def stopSession(
    request: ChargeSessionApi.StopChargeSessionRequest
  ): Task[ChargeSessionApi.StopChargeSessionResponse] =
    for {
      uuid         <- ZIO.succeed(java.util.UUID.fromString(request.sessionId))
      maybeSession <- chargeSessionRepository.get(uuid)
      session <-
        ZIO.fromOption(maybeSession).orElseFail(new IllegalArgumentException(s"Session with id ${uuid} not found"))
      now <- zio.Clock.instant

...
import io.tuliplogic.ziotoolbox.tracing.commons.{ServerTracerAlgebra, ServerTracerBaseInterpreter}
...
trait TapirTracingInterpretation { interpretation =>
  ...
  protected def serverLogicTracing[R, SECURITY_INPUT, PRINCIPAL, INPUT, ERROR_OUTPUT, OUTPUT, C](e: ZPartialServerEndpoint[R, SECURITY_INPUT, PRINCIPAL, INPUT, ERROR_OUTPUT, OUTPUT, C])(spanName: String)(logic: PRINCIPAL => INPUT => ZIO[R, ERROR_OUTPUT, OUTPUT]): sttp.tapir.ztapir.ZServerEndpoint[R, C]
...
object TapirTracingInterpretation {
  val defaultTapirServerTracerAlgebra = new ServerTracerAlgebra[Any, Any]{}
  def layer(tracerAlgebra: ServerTracerAlgebra[Any, Any] = defaultTapirServerTracerAlgebra): ZLayer[Baggage with Tracing, Nothing, TapirTracingInterpretation] =
    ZLayer.fromZIO {
      for {
...
class TapirTracingInterpreter(
  val tracerAlgebra: ServerTracerAlgebra[Any, Any],
  val tracing: Tracing,
  val baggage: Baggage,
) extends ServerTracerBaseInterpreter[Any, Any, List[Header], TapirTracingInterpretation] {

  override def transportToCarrier(headers: List[Header]): IncomingContextCarrier[Map[String, String]] = new IncomingContextCarrier[Map[String, String]] {
    val safeHeaders =
  ...
  override def interpretation: UIO[TapirTracingInterpretation] = ZIO.succeed(
    new TapirTracingInterpretation { interpretation =>
      override def zServerLogicTracing[R, SECURITY_INPUT, INPUT, ERROR_OUTPUT, OUTPUT, C](e: Endpoint[SECURITY_INPUT, INPUT, ERROR_OUTPUT, OUTPUT, C])(spanName: String)(logic: INPUT => ZIO[R, ERROR_OUTPUT, OUTPUT])(implicit aIsUnit: SECURITY_INPUT =:= Unit): sttp.tapir.ztapir.ZServerEndpoint[R, C] = {
      ...
      override def serverLogicTracing[R, SECURITY_INPUT, PRINCIPAL, INPUT, ERROR_OUTPUT, OUTPUT, C](e: ZPartialServerEndpoint[R, SECURITY_INPUT, PRINCIPAL, INPUT, ERROR_OUTPUT, OUTPUT, C])(spanName: String)(logic: PRINCIPAL => INPUT => ZIO[R, ERROR_OUTPUT, OUTPUT]): sttp.tapir.ztapir.ZServerEndpoint[R, C] = {
        val endpointWithRequestHeaders = e.in(sttp.tapir.headers)
